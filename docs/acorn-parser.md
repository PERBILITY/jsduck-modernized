...
2. Set `JSDUCK_PARSER=acorn`.
3. Ensure Node.js + acorn are available wherever JSDuck runs (e.g. add them to
   the JSDuck Docker image).
...
- **Proof-of-concept maturity.** Validated against the examples in
  [poc/](../poc/) (a classic ES5 `Ext.define` class and an equivalent modern
  one). Not yet exercised against a full large codebase — rare modern syntax
  appearing on the *documented surface* (not inside method bodies) may need
  small additions to the member detectors.
